    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (existed == null) {
                ...
                if (rs > 0) {
                    int conversation = ConversationDAO.getConversationID(topic);
                    Message m = new Message(sender_id, receiver_id, content, conversation);
                    MessageDAO.createMessage(m);
                    request.setAttribute("CONVER_ID", conversation);
