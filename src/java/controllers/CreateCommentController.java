...
public class CreateCommentController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String post_idString = request.getParameter("post-id").trim();
            ...
            int parent_id = 0;
            if (parent_idString != null && parent_idString != "NULL") {
                parent_id = Integer.parseInt(parent_idString);
            }
            int rs = 0;
            if (author_id != null) {
                Comment comment = new Comment(author_id, post_id, content, parent_id);
                rs = CommentDAO.createComment(comment);
                if (rs > 0) {
                    request.getRequestDispatcher("DispatcherController?action=thread&id=" + post_id).forward(request, response);
                } else {
